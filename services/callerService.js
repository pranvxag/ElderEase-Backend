            ...
            try {
                ...
                if (['no-answer', 'busy', 'failed'].includes(status)) {
                    await twilioClient.messages.create({
                        body: limitSmsText(`💊 Time to take ${medicineName} ⏰`),
                        from: process.env.TWILIO_PHONE_NUMBER,
                        to: phoneNumber
                    });
...
async function sendCaregiverSMS(uid, medicineName, status) {
    try {
        ...
        if (userDoc.exists) {
            ...
            if (status === 'taken') {
                messageBodyCaregiver = `🎉✅ ${elderName} took ${medicineName}!`;
                messageBodyUser = `🎉✅ You logged taking ${medicineName}!`;
            } else if (status === 'not taken') {
                messageBodyCaregiver = `⚠️ ${elderName} MISSED ${medicineName}. Check soon!`;
                messageBodyUser = `🔔 Not taken. Reminder in 15 min ⏰`;
            } else if (status === 'no response') {
                messageBodyCaregiver = `❓ ${elderName} didn't respond ⚠️`;
                messageBodyUser = `🔔 Missed reminder. We'll call again!`;
            }

            messageBodyCaregiver = limitSmsText(messageBodyCaregiver);
            messageBodyUser = limitSmsText(messageBodyUser);
...
async function generateWeeklyReport(uid) {
    ...
    try {
        ...
        // SMS-friendly version (under 150 chars for Twilio trial)
        const smsText = limitSmsText(`📊 ${elderName}: Meds ${adherence}% ✅ Sugar ${avgMorning > 0 ? avgMorning : '-'}/${avgEvening > 0 ? avgEvening : '-'} mg/dL`);
