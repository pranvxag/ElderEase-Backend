...
async function generateWeeklyReport(uid) {
    ...
    try {
        ...
        // SMS-friendly version (under 160 chars for Twilio trial)
        const smsText = `ElderEase: ${elderName} weekly report (${startDate} to ${endDate}). Meds: ${totalTaken}/${totalScheduled} (${adherence}%). Sugar avg: ${avgMorning > 0 ? avgMorning : '-'}/${avgEvening > 0 ? avgEvening : '-'} mg/dL`;
