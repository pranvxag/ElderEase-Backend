...
async function triggerMedicineReminderCall(uid, entry) {
    ...
    const language = profile?.preferredLanguage ?? entry?.lang ?? 'en';
    ...
    await twilioClient.calls.create({
        ...
        statusCallback: `${serverUrl}/ivr/call-status?uid=${encodeURIComponent(uid)}&entryId=${encodeURIComponent(entry.id)}&medicineName=${encodeURIComponent(medicineName)}&logDate=${encodeURIComponent(logDate)}`,
